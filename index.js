...
setInterval(() => {
  try {
    fs.copyFileSync('./data/ravedad.db', './data/ravedad.backup.db');
  } catch (e) {
...
function sanitizeLabel(text) {
  // Discord label max 100; keep a little margin + strip control chars
  const s = (text ?? 'Unknown').toString().replace(/[\u0000-\u001F\u007F]/g, '').trim();
  if (!s) return 'Unknown';
  return s.length > 90 ? s.slice(0, 89) + '…' : s;
}

/**
 * Mobile-crash hardening:
 * - NEVER use the human label as the menu "value"
 * - Use small deterministic values like "select_micro_1|7"
...
function buildMenuRow(draft, menuId, placeholder, labels, { includeBack = false, backTo = null, includeSkip = false } = {}) {
  ...
  const options = [];
  if (includeSkip) {
    options.push(new StringSelectMenuOptionBuilder().setLabel('🚫 No Secondary Genre (Skip)').setValue('SKIP'));
  }
  ...
  if (includeBack && backTo) {
    rows.push(
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`back_to_${backTo}`).setLabel('⬅️ Back').setStyle(ButtonStyle.Secondary)
      )
    );
  }
...
function resolveMenuSelection(draft, menuId, selectedValue) {
  ...
  if (!selectedValue || typeof selectedValue !== 'string') return null;

  const parts = selectedValue.split('|');
  if (parts.length !== 2) return null;

  const [valMenuId, idxStr] = parts;
  if (valMenuId !== menuId) return null;

  const idx = Number(idxStr);
  if (!Number.isInteger(idx) || idx < 0) return null;

  const list = draft?.optionLists?.[menuId];
...
function getUser(userId) {
  ...
  if (!user) {
    db.prepare(
      'INSERT INTO users (id, credits, lifetime_points, daily_points, last_active) VALUES (?, 10, 0, 0, ?)'
...
function checkDailyLimit(userId) {
  ...
  const count = db
    .prepare('SELECT COUNT(*) as count FROM songs WHERE user_id = ? AND timestamp > ?')
...
function getSubmissionCooldown(userId) {
  ...
  const songs = db
    .prepare('SELECT timestamp FROM songs WHERE user_id = ? ORDER BY timestamp DESC LIMIT ?')
    .all(userId, limit);
...
function addPoints(userId, amount = 2) {
  ...
  if (user.credits >= WALLET_CAP) {
    db.prepare(
      'UPDATE users SET lifetime_points = lifetime_points + ?, daily_points = daily_points + ?, last_active = ? WHERE id = ?'
    ).run(amount, amount, today, userId);
    return { earned: false, reason: 'wallet_cap' };
  }
  db.prepare(
    'UPDATE users SET credits = credits + ?, lifetime_points = lifetime_points + ?, daily_points = daily_points + ?, last_active = ? WHERE id = ?'
  ).run(amount, amount, amount, today, userId);
...
function modifyUpvotes(songId, amount) {
  db.prepare('UPDATE songs SET upvotes = upvotes + ? WHERE id = ?').run(amount, songId);
}
function incrementViews(songId) {
  db.prepare('UPDATE songs SET views = views + 1 WHERE id = ?').run(songId);
}
function getSongStats(songId) {
  return db
    .prepare('SELECT upvotes, views, message_id, channel_id, user_id, description, artist_name, title, tags, url FROM songs WHERE id = ?')
    .get(songId);
}
...
function canUserVote(userId, songId, newPoints) {
  ...
  const record = db
    .prepare('SELECT SUM(amount) as total FROM votes WHERE voter_id = ? AND song_id = ? AND amount > 0')
    .get(userId, songId);
...
function recordVote(userId, songId, amount) {
  db.prepare('INSERT INTO votes (song_id, voter_id, type, timestamp, amount) VALUES (?, ?, ?, ?, ?)').run(
    songId,
    userId,
    'VOTE',
    Date.now(),
...
// Missing in your uploaded file — adding it back:
function generateSongListEmbed(targetUser, songs) {
  const title = `🎵 Last 5 Submissions: ${targetUser.username}`;
  if (!songs || songs.length === 0) {
    return new EmbedBuilder().setColor(0x3498db).setTitle(title).setDescription('No submissions yet.');
  }
  ...
  const lines = songs.map((s, i) => {
    ...
    const genreLine =
      tags.length >= 2
        ? `${tags[0]} > ${tags[1]}${tags[2] && tags[2] !== 'SKIP' ? ` | ${tags[2]} > ${tags[3] || ''}` : ''}`
        : '(No genres)';

    const t = s.title ? truncate(s.title, 40) : `Track ${s.id}`;
    const a = s.artist_name ? truncate(s.artist_name, 30) : 'Unknown';
    return `${getRankIcon(i)} **${a}** — **${t}**\n${genreLine}\n[Listen](${s.url}) • 🔥 ${s.upvotes || 0} • 👀 ${s.views || 0}`;
  });

  return new EmbedBuilder().setColor(0x3498db).setTitle(title).setDescription(lines.join('\n\n'));
}


function safeParseTags(tagsText) {
  try { return JSON.parse(tagsText || '[]'); } catch (e) { return []; }
}

function formatGenreLine(tags) {
  if (!Array.isArray(tags) || tags.length < 2) return '(No genres)';
  const primary = `${tags[0]} > ${tags[1]}`;
  const secondary = (tags[2] && tags[2] !== 'SKIP') ? ` | ${tags[2]} > ${tags[3] || ''}` : '';
  return `${primary}${secondary}`.replace(/\s+>\s*\|/g, ' |'); // tiny cleanup if missing micro2
}

function generateMySongsEmbed(targetUser, songs, totals, best) {
  const totalSongs = totals?.songs || 0;
  const totalUpvotes = totals?.upvotes || 0;
  const totalViews = totals?.views || 0;
  const avgScore = totalSongs > 0 ? (totalUpvotes / totalSongs).toFixed(2) : '0.00';
  const avgViews = totalSongs > 0 ? (totalViews / totalSongs).toFixed(2) : '0.00';

  const embed = new EmbedBuilder()
    .setColor(0x3498db)
    .setTitle(`🎵 My Songs: ${targetUser.username}`)
    .setThumbnail(targetUser.displayAvatarURL())
    .addFields(
      { name: 'Submissions', value: `**${totalSongs}**`, inline: true },
      { name: 'Total Score', value: `🔥 **${totalUpvotes}**`, inline: true },
      { name: 'Total Views', value: `👀 **${totalViews}**`, inline: true },
      { name: 'Avg Score/Song', value: `🔥 **${avgScore}**`, inline: true },
      { name: 'Avg Views/Song', value: `👀 **${avgViews}**`, inline: true },
      { name: '\u200B', value: '\u200B', inline: true }
    );

  if (best && best.url) {
    const bestTitle = best.title ? truncate(best.title, 35) : `Track ${best.id}`;
    const bestArtist = best.artist_name ? ` by **${truncate(best.artist_name, 30)}**` : '';
    embed.addFields({
      name: '🏆 Top Track',
      value: `**[${bestTitle}](${best.url})**${bestArtist}\n🔥 ${best.upvotes || 0} • 👀 ${best.views || 0} • ID: ${best.id}`,
      inline: false
    });
  }

  if (!songs || songs.length === 0) {
    embed.setDescription('No submissions yet. Use `/submit` to drop your first track.');
    return embed;
  }

  const lines = songs.slice(0, 10).map((s, i) => {
    const tags = safeParseTags(s.tags);
    const genreLine = formatGenreLine(tags);
    const title = s.title ? truncate(s.title, 24) : truncate(s.description, 24);
    const artist = s.artist_name ? truncate(s.artist_name, 18) : '';
    const name = artist ? `${artist} - ${title}` : title;
    const score = s.upvotes || 0;
    const views = s.views || 0;
    const when = s.timestamp ? `<t:${Math.floor(s.timestamp / 1000)}:R>` : '';
    return `${i + 1}. **${name}**\n${genreLine}\n🔥 ${score} • 👀 ${views}${when ? ` • ${when}` : ''}\n[Listen](${s.url}) • ID: ${s.id}`;
  });
...
async function updateLeaderboard(guild) {
  ...
  const weeklyReviews = db
    .prepare('SELECT COUNT(*) as count FROM reviews WHERE timestamp > ?')
  ...
  const embedStats = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '\u200B', value: '\u200B', inline: true },
      { name: 'Weekly Songs', value: `**${weeklySongs}**`, inline: true },
      { name: 'Weekly Reviews', value: `**${weeklyReviews}**`, inline: true },
      { name: '\u200B', value: '\u200B', inline: true }
    );

  const topCritics = db.prepare('SELECT id, lifetime_points FROM users ORDER BY lifetime_points DESC LIMIT 10').all();
  const criticList =
    topCritics.map((u, i) => `${getRankIcon(i)} <@${u.id}> • **${u.lifetime_points}**`).join('\n') || 'No data.';
  const topArtists = db.prepare('SELECT user_id, COUNT(*) as count FROM songs GROUP BY user_id ORDER BY count DESC LIMIT 10').all();
  const artistList =
    topArtists.map((u, i) => `${getRankIcon(i)} <@${u.user_id}> • **${u.count}**`).join('\n') || 'No data.';
  const embedLifePeople = new EmbedBuilder()
    .setColor(0xffd700)
    .setTitle('🏆 LIFETIME: PEOPLE')
    .addFields(
      { name: '👑 Top Critics (Pts)', value: criticList, inline: true },
      { name: '🎨 Top Artists (Vol)', value: artistList, inline: true }
    );
  ...
  const songList =
    topSongs
      .map((s, i) => {
        ...
        return `${getRankIcon(i)} ${artist} - [${displayTitle}](${s.url}) • 🔥 **${s.upvotes}**`;
      })
      .join('\n') || 'No data.';
  const embedLifeTracks = new EmbedBuilder().setColor(0xffa500).setTitle('🔥 LIFETIME: TRACKS').setDescription(songList);

  const weekCritics = db
    .prepare('SELECT user_id as id, COUNT(*) * 2 as score FROM reviews WHERE timestamp > ? GROUP BY user_id ORDER BY score DESC LIMIT 10')
    .all(sevenDaysAgo);
  const weekCriticList =
    weekCritics.map((u, i) => `${getRankIcon(i)} <@${u.id}> • **${u.score}**`).join('\n') || 'No data.';
  const weekArtists = db
    .prepare('SELECT user_id, COUNT(*) as count FROM songs WHERE timestamp > ? GROUP BY user_id ORDER BY count DESC LIMIT 10')
    .all(sevenDaysAgo);
  const weekArtistList =
    weekArtists.map((u, i) => `${getRankIcon(i)} <@${u.user_id}> • **${u.count}**`).join('\n') || 'No data.';
  const embedWeekPeople = new EmbedBuilder()
    .setColor(0x00ff00)
    .setTitle('📅 WEEKLY: PEOPLE')
    .addFields(
      { name: '🚀 Top Critics (Pts)', value: weekCriticList, inline: true },
      { name: '🎨 Top Artists (Vol)', value: weekArtistList, inline: true }
    );

  const weekSongsRaw = db
    .prepare('SELECT song_id, SUM(amount) as score FROM votes WHERE timestamp > ? AND amount > 0 GROUP BY song_id ORDER BY score DESC LIMIT 10')
    .all(sevenDaysAgo);
  const weekSongList =
    weekSongsRaw
      .map((stat, i) => {
        const song = db.prepare('SELECT url, title, artist_name FROM songs WHERE id = ?').get(stat.song_id);
        if (!song) return `${getRankIcon(i)} Unknown`;
        const artist = song.artist_name ? `**${song.artist_name}**` : 'Unknown';
        const displayTitle = song.title ? truncate(song.title, 20) : 'Track';
        return `${getRankIcon(i)} ${artist} - [${displayTitle}](${song.url}) • 🔥 **+${stat.score}**`;
      })
      .join('\n') || 'No data.';
  const embedWeekTracks = new EmbedBuilder()
    .setColor(0x00ff00)
    .setTitle('📈 WEEKLY: TRACKS')
    .setDescription(weekSongList)
  ...
  await sendOrEdit('📊 SERVER STATS', embedStats);
  await sendOrEdit('🏆 LIFETIME: PEOPLE', embedLifePeople);
  await sendOrEdit('🔥 LIFETIME: TRACKS', embedLifeTracks);
  await sendOrEdit('📅 WEEKLY: PEOPLE', embedWeekPeople);
  await sendOrEdit('📈 WEEKLY: TRACKS', embedWeekTracks);
}
...
async function updatePublicEmbed(guild, songId) {
  ...
  const channelId = song.channel_id || CHANNEL_LEGACY;
  ...
  try {
    ...
    if (message) {
      ...
      const titleField = song.title ? `**Title:** ${song.title}\n` : '';
      const newEmbed = new EmbedBuilder()
        .setColor(0x0099ff)
        .setTitle('🔥 Fresh Drop Alert')
        .setDescription(
          `**User:** <@${song.user_id}>\n${titleField}${artistField}**Genres:**\n${primaryDisplay}${secondaryDisplay}\n\n**Description:**\n${song.description}`
        )
    ...
    console.error(`Update Embed Failed for Song ${songId}:`, e);
  }
...
async function finalizeSubmission(interaction, draft) {
  if (!spendCredits(interaction.user.id, SUBMISSION_COST)) {
    return interaction.editReply({
      content: `❌ Transaction Failed. You need ${SUBMISSION_COST} Credits. Review tracks to earn more.`,
      components: []
    });
  }
  ...
  const stmt = db.prepare(
    'INSERT INTO songs (user_id, url, description, tags, timestamp, artist_name, channel_id, title) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  );
  ...
  if (channel) {
    ...
    const sentMsg = await channel.send({
      embeds: [embed],
      components: [new ActionRowBuilder().addComponents(listenBtn, reportBtn)]
    });
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName !== 'admin-delete' && commandCooldowns.has(userId)) {
      if (now < commandCooldowns.get(userId) + 3000) {
        return interaction.reply({ content: `⏳ Wait 3 seconds.`, ephemeral: true });
      }
    }
    ...
    if (interaction.commandName === 'admin-delete') {
      ...
      if (channel && song.message_id) {
        try {
          ...
          console.log('Message/Thread already gone.');
        }
      }

      db.prepare('DELETE FROM songs WHERE id = ?').run(songId);
      return interaction.reply({ content: `🗑️ Terminated. Song ID ${songId} deleted.`, ephemeral: true });
    }
    ...
    if (interaction.commandName === 'admin-add-points') {
      ...
      return interaction.reply({ content: `✅ Stimulus Applied. Given ${amount} Credits & Lifetime Pts to ${target.username}.`, ephemeral: true });
    }
    ...
    if (interaction.commandName === 'admin-set-bonus') {
      ...
      return interaction.reply({ content: `✅ Limit Updated. ${target.username} now has +${slots} extra daily submissions.`, ephemeral: true });
    }
    ...
    if (interaction.commandName === 'weekly-report') {
      ...
      return interaction.reply({ content: '✅ Boards refreshed.', ephemeral: true });
    }
    ...
    if (interaction.commandName === 'suspend') {
      ...
      return interaction.reply({ content: `🚫 User Suspended.\nTarget: ${target.username}\nDuration: ${hours} Hours\nReason: ${reason}`, ephemeral: false });
    }
    ...
    if (interaction.commandName === 'stage') {
      ...
      if (existing) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`**${existing.title || 'Track'}** by ${existing.artist_name || 'Unknown'}\n${existing.description}`)
          .addFields({ name: 'Listen', value: link })
          .setFooter({ text: `ID: ${existing.id} | Score: ${existing.upvotes}` });

        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`vote_1_${existing.id}`).setLabel('🔥 Banger (+1)').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`scribe_${existing.id}`).setLabel('📝 Scribe Note').setStyle(ButtonStyle.Secondary)
        );

        await logChannel.send({ embeds: [embed], components: [row] });
        return interaction.reply({ content: '✅ Queued existing track.', ephemeral: true });
      }
      ...
      if (status.count >= status.limit) {
        const cooldownTimestamp = getSubmissionCooldown(interaction.user.id);
        return interaction.reply({
          content: `🛑 Daily Limit Reached! You cannot stage NEW songs until <t:${cooldownTimestamp}:R>.`,
          ephemeral: true
        });
      }
    ...
    if (interaction.commandName === 'profile') {
      ...
      if (!isSelf && !hasPower) return interaction.reply({ content: '❌ Only Moderators can inspect other users.', ephemeral: true });
    ...
    if (interaction.commandName === 'share-profile') {
      ...
      return interaction.reply({ content: '📢 Flexing Stats!', embeds: [embed], ephemeral: false });
    }
    ...
    function generateProfileEmbed(user, stats, displayDaily, subStatus, cooldownTimestamp, avatarUrl, username) {
      ...
      let ratio = 0;
      if (stats.songs > 0) ratio = (stats.reviews / stats.songs).toFixed(1);
      else if (stats.reviews > 0) ratio = '∞';
    ...
    if (interaction.commandName === 'init-welcome') {
      ...
      const embed = new EmbedBuilder()
        .setColor(0xff00ff)
        .setTitle('🤖 Welcome to the Future of Music')
        .setDescription(
          `We are a mature community of AI music creators dedicated to honest feedback and growth.\n\n**COMMUNITY RULES:**\n1. **Respect Everyone:** No hate speech, harassment, or toxicity.\n2. **Give to Get:** You must review songs to earn credits.\n3. **Honesty:** Low-effort spam reviews will result in a ban.\n4. **No Spam:** Don't DM members with self-promotion.\n5. **18+ Only:** Some Content is Mature, 18+ years of age required.\n\n*By clicking below, you agree to these terms.*`
        )
        .setImage('https://media.discordapp.net/attachments/123456789/123456789/banner.png');

      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('accept_tos').setLabel('✅ I Accept & Enter').setStyle(ButtonStyle.Success)
      );

      await interaction.channel.send({ embeds: [embed], components: [row] });
      return interaction.reply({ content: 'Gatekeeper initialized.', ephemeral: true });
    }
    ...
    if (interaction.commandName === 'share-songs') {
      ...
      const songs = db.prepare('SELECT * FROM songs WHERE user_id = ? ORDER BY timestamp DESC LIMIT 5').all(targetUser.id);
      const embed = generateSongListEmbed(targetUser, songs);
      return interaction.reply({ content: `🎵 Recent Tracks by ${targetUser.username}`, embeds: [embed], ephemeral: false });
    }


        if (interaction.commandName === 'my-songs') {
            const songs = db.prepare('SELECT * FROM songs WHERE user_id = ? ORDER BY timestamp DESC LIMIT 10').all(interaction.user.id);
            const totals = db.prepare('SELECT COUNT(*) as songs, COALESCE(SUM(upvotes),0) as upvotes, COALESCE(SUM(views),0) as views FROM songs WHERE user_id = ?').get(interaction.user.id);
            const best = db.prepare('SELECT * FROM songs WHERE user_id = ? ORDER BY upvotes DESC, views DESC, timestamp DESC LIMIT 1').get(interaction.user.id);
            const embed = generateMySongsEmbed(interaction.user, songs, totals, best);
            await interaction.reply({ embeds: [embed], ephemeral: true });
        }

        if (interaction.commandName === 'share-my-songs') {
            const songs = db.prepare('SELECT * FROM songs WHERE user_id = ? ORDER BY timestamp DESC LIMIT 10').all(interaction.user.id);
            const totals = db.prepare('SELECT COUNT(*) as songs, COALESCE(SUM(upvotes),0) as upvotes, COALESCE(SUM(views),0) as views FROM songs WHERE user_id = ?').get(interaction.user.id);
            const best = db.prepare('SELECT * FROM songs WHERE user_id = ? ORDER BY upvotes DESC, views DESC, timestamp DESC LIMIT 1').get(interaction.user.id);
            const embed = generateMySongsEmbed(interaction.user, songs, totals, best);
            await interaction.reply({ content: `🎵 **${interaction.user.username}**'s track stats:`, embeds: [embed], ephemeral: false });
        }
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (!draft) {
      return interaction.reply({
        content: '❌ Session Expired. The bot may have restarted. Please run `/submit` again.',
        ephemeral: true
      });
    }

    // Always acknowledge quickly to avoid client weirdness
    await interaction.deferUpdate();

    if (interaction.customId === 'stage_select_genre') {
      const chosen = resolveMenuSelection(draft, 'stage_select_genre', interaction.values?.[0]);
      if (!chosen) return interaction.editReply({ content: '❌ Selection error. Please try again.', components: [] });

      const genre = chosen;
      const targetChannelId = CHANNEL_ROUTER[genre] || CHANNEL_LEGACY;
      const stmt = db.prepare(
        'INSERT INTO songs (user_id, url, description, tags, timestamp, title, artist_name, channel_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
      );
      ...
      const embed2 = new EmbedBuilder()
        .setColor(0xff00ff)
        .setTitle('🔴 NOW PLAYING')
        .setDescription(`**${draft.title}** by ${draft.artist_name}`)
        .addFields({ name: 'Listen', value: draft.link })
        .setFooter({ text: `ID: ${songId}` });

      const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`vote_1_${songId}`).setLabel('🔥 Banger (+1)').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`scribe_${songId}`).setLabel('📝 Scribe Note').setStyle(ButtonStyle.Secondary)
      );

      await logChannel.send({ embeds: [embed2], components: [row2] });

      return interaction.editReply({ content: '✅ On Stage! Posted to Session Log AND Public Channel.', components: [] });
    }

    if (interaction.customId === 'select_macro_1') {
      const chosen = resolveMenuSelection(draft, 'select_macro_1', interaction.values?.[0]);
      if (!chosen) return interaction.editReply({ content: '❌ Selection error. Please try again.', components: [] });
      ...
      if (!Array.isArray(subGenres) || subGenres.length === 0) {
        return interaction.editReply({ content: '❌ Configuration Error: No sub-genres found.', components: [] });
      }
      ...
      return interaction.editReply({
        content: `Step 2/4: Select specific style for ${draft.macro1}`,
        components: rows
      });
    }

    if (interaction.customId === 'select_micro_1') {
      const chosen = resolveMenuSelection(draft, 'select_micro_1', interaction.values?.[0]);
      if (!chosen) return interaction.editReply({ content: '❌ Selection error. Please try again.', components: [] });
      ...
      const rows = buildMenuRow(draft, 'select_macro_2', 'Select Secondary Category', macros, {
        includeSkip: true,
        includeBack: true,
        backTo: 'micro_1'
      });
      ...
      return interaction.editReply({
        content: `Step 3/4: Select a Secondary Genre (or Skip)`,
        components: rows
      });
    }

    if (interaction.customId === 'select_macro_2') {
      ...
      const chosen = resolveMenuSelection(draft, 'select_macro_2', selected);
      if (!chosen) return interaction.editReply({ content: '❌ Selection error. Please try again.', components: [] });

      draft.macro2 = chosen;
      draftSubmissions.set(interaction.user.id, draft);

      const subGenres = taxonomy[draft.macro2] || [];
      if (!Array.isArray(subGenres) || subGenres.length === 0) {
        return interaction.editReply({ content: '❌ Configuration Error: No sub-genres found.', components: [] });
      }
      ...
      return interaction.editReply({
        content: `Step 4/4: Select specific style for ${draft.macro2}`,
        components: rows
      });
    }

    if (interaction.customId === 'select_micro_2') {
      const chosen = resolveMenuSelection(draft, 'select_micro_2', interaction.values?.[0]);
      if (!chosen) return interaction.editReply({ content: '❌ Selection error. Please try again.', components: [] });
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'submission_modal') {
      ...
      const draft = { link, description: desc, artist_name: artist, title: title, optionLists: {} };
      draftSubmissions.set(interaction.user.id, draft);
      ...
      return interaction.reply({
        content: `Step 1/4: Select Primary Genre`,
        components: rows,
        ephemeral: true
      });
    }
    ...
    if (interaction.customId === 'stage_modal') {
      ...
      return interaction.reply({ content: `Select Genre for the Stage:`, components: rows, ephemeral: true });
    }
    ...
    if (interaction.customId.startsWith('scribe_submit_')) {
      ...
      if (channel && song.message_id) {
        ...
        if (message) {
          ...
          if (!thread) {
            thread = await message.startThread({ name: `💬 Reviews: ${song.title || 'Track'}`, autoArchiveDuration: 60 });
          }
    ...
    if (interaction.customId.startsWith('review_submit_')) {
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`vote_1_${songId}`).setLabel('+1 Vote (Cost: 1)').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId(`vote_2_${songId}`).setLabel('+2 Votes (Cost: 2)').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId(`vote_3_${songId}`).setLabel('🔥 God Mode +3 (Cost: 3)').setStyle(ButtonStyle.Success)
      );
      if (user.lifetime_points >= 50) {
        row.addComponents(new ButtonBuilder().setCustomId(`vote_neg1_${songId}`).setLabel('👎 Dislike (Cost: 3)').setStyle(ButtonStyle.Danger));
      }
      ...
      if (song && song.message_id) {
        try {
          ...
          if (message && message.thread) {
            await message.thread.send(`⭐ <@${interaction.user.id}> left a review for <@${song.user_id}>:\n"${reviewText}"`);
          }
        } catch (e) {
  ...
  if (interaction.isButton()) {
    ...
    if (action === 'back') {
      ...
      // customId format: back_to_macro_1, back_to_micro_1, back_to_macro_2
      const backTarget = parts.slice(2).join('_'); // e.g. "macro_1"

      if (backTarget === 'macro_1') {
        const macros = Object.keys(taxonomy);
        const rows = buildMenuRow(draft, 'select_macro_1', 'Select Primary Category', macros);
        draftSubmissions.set(interaction.user.id, draft);
        return interaction.editReply({ content: `Step 1/4: Select Primary Genre`, components: rows });
      }
    ...
    if (action === 'vote') {
      ...
      const cost = type === 'neg1' ? 3 : parseInt(type, 10);
      const pointsToAdd = type === 'neg1' ? -1 : parseInt(type, 10);
